      ...
      if (products) {
        let newProducts = products.filter( function (item) {
          if (item.id == req.params.id) {
            ...
            if (item.image) {rwdJson.deleteJSON(pathProductos + item.image)};
            if (item.leftImage) {rwdJson.deleteJSON(pathProductos + item.leftImage)};
            if (item.rightImage) {rwdJson.deleteJSON(pathProductos + item.rightImage)};
            if (item.upperImage) {rwdJson.deleteJSON(pathProductos + item.upperImage)};
            if (item.lowerImage) {rwdJson.deleteJSON(pathProductos + item.lowerImage)};
            return false;
          } else {
            return true;
          }
